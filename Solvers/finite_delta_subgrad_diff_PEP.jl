...
For each `N` in `N_start:N_step:N_stop`, run `optimize_δ_for_fixed_α` with
...
function run_obj_vs_N_sweep(
    ...
    return N_vals, true_Lip, theory_SGM, true_BSD
end
...
N_vals, true_Lip, theory_SGM, true_BSD = run_obj_vs_N_sweep(L, μ, R, p; Lip_scaling = 1/sqrt(2))
