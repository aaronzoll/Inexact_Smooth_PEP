...
function run_N_opti(N, R, β, p)
    ...
    min_δ = Optim.minimizer(result)[1]
...
function plot_N_rates(R, β, p, k)
    ...
    for (cnt, N) in enumerate(X)
        δ_set[cnt], Y1[cnt] = run_N_opti(N, R, β, p) # PRINTING OFF
        if mod(N, 50) == 0
            println("trial: $N")
        end
        a, b = get_sequences(N)
    ...
    plot(X, Y1, labels="same δ")
    return δ_set, coeff, δ_calc, Y1
end
...
function plot_p_coeff(R, β, k, N=100)
    ...
    for (cnt, p) in enumerate(p_range)
        min_δ, rate = run_N_opti(N, R, β, p) # PRINTING OFF
        min_δ_sets[cnt] = min_δ[1]
    ...
    return p_range, coeffs, min_δ_sets
end
...
p_range, coeffs, min_δ_sets = plot_p_coeff(R, β, k, N)
plot(p_range, coeffs)
