#!/usr/bin/env julia
# ============================================================
# Infimal convolution for convex (possibly NONDIFFERENTIABLE) f,g
#
# h(x) = (f ⊞ g)(x) := inf_y  f(y) + g(x - y)
#
# IMPORTANT: We DO NOT assume differentiability.
# - Default inner solver is derivative-free (Optim.NelderMead).
# - Optionally, you may supply subgradient oracles for f and g and
#   we’ll use a simple subgradient method (no gradient-based solver).
#
# Plotting supports d=1 or d=2 directly with Makie.
# Always plots h, and optionally plots f and/or g.
#
# Also includes:
#   p(x; X, F, G) = max_i ( F[i] + ⟨G[i], x - X[i]⟩ )
# where X[i] and G[i] can be vectors (or scalars in 1D).
# ============================================================
...
# Non-smooth support:
#   - derivative-free NelderMead (default)
#   - optional subgradient method if subgrad_f/subgrad_g provided
# -------------------------

"""
derivative_free_minimize(ϕ, y0; restarts, radius, seed)

Uses NelderMead (no gradients). Adds a few random restarts around y0 to reduce
the chance of a bad simplex in higher dimension.
...
No line search, no gradient-based solver.
...
Warm-start cache is used by default.
"""
...
println("max_affine([0.4]) = ", max_affine([0.4], X, F, G))
